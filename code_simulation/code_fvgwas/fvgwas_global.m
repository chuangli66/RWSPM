...
for s = 1:7
    ...
    for l = 1:100
        ...
        X = randn(n,5);
        img_path = root + '/sim_'+ num2str(l) + '/y.mat';
        load(img_path);
        YW = Y;
   ...
   l
    ind_name = strcat(save_root,'fvgwas_global_set',num2str(s),'.csv');
    writematrix(p_vec,ind_name);

    mean(p_vec < 0.05)
    fprintf('+++++++GSIS procedure is finished!!+++++++\n');
end
...
def sKPCR_adptive_regression1(pcs,pheno,numperms):
    N,P=pcs.shape
    # yperms: 
    #   the first one is pheno
    #   the 2nd to end are permuted pheno
...
def sKPCR_run_full_analysis(mat, x, K_components,numperms=199):
    ...
    return pv
## k is the number of Setting 1-7
k=1
## File loading path
root = "/.../Setting" + str(k) + "/sim_"
## file storage path
save_root = "/.../Setting"+ str(k) + "/"
...
for l in range(1,100):
  ...
  # if os.path.exists(p+'Y_slide_width27.npy'):
    x = pyreadr.read_r(p+'x.RData')["x"]
    n = x.shape[0]
    x = x.values.reshape(n)
    
    lqd = np.load(p+'Y_slide.npy')
    LQD = np.zeros((6534,n,9))
    for i in range(6534):
      s = n*i
      e = n*(i+1)
      LQD[i,:,:] = lqd[s:e,]
    ...
    for j in range(m):
      # if(j % 10 == 0): print(j)
      # start = time.time()
      pvals[j] = sKPCR_run_full_analysis(LQD[j,:,:], x, 5)
      if pvals[j] == 0:
          pvals[j] = sKPCR_run_full_analysis(LQD[j,:,:], x, 5,9999)
    o = np.column_stack((range(1,m+1), pvals))
    np.savetxt(save_root+'test_skpcr'+str(l)+'.txt', o)
    end = time.time() # 77s  permute times 9999
    print(str(np.round(end-start,2)/3600)+' h')
...
for l in range(1,101):
    ...
    n = x.shape[0]
    x = x.values.reshape(n)
    y = pyreadr.read_r(p+'y.RData')["y"]
    pvals[l-1] = sKPCR_run_full_analysis(y, x, 5)
    print(str(k)+"_"+str(l) +":"+ str(pvals[l-1]))
o = np.column_stack((range(1,101), pvals))
np.savetxt(save_root+'skpcr_global_set'+str(k)+'.txt', o)
end = time.time() # 77s  permute times 9999
